            ...
            >
              ...
              <div className="rounded-lg bg-bg-elevated/60 border border-line-subtle p-2.5 space-y-1.5">
                <div className="flex items-center justify-between gap-2 mb-1">
                  <p className="text-[11px] font-medium text-fg-muted">Token 过期时间</p>
                  <button
                    onClick={refreshAuth}
                    disabled={isRefreshingAuth}
                    className="flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-medium text-primary bg-primary-muted hover:bg-primary/15 transition-colors disabled:opacity-50"
                  >
                    <RefreshCw className={cn('w-3 h-3', isRefreshingAuth && 'animate-spin')} />
                    {isRefreshingAuth ? '刷新中' : '刷新 Token'}
                  </button>
                </div>
