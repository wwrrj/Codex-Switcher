...
#[tauri::command]
pub async fn clear_proxy_events() -> Result<ProxyState, String> {
    blocking(move || {
        let actual_home = actual_home()?;
        crate::proxy::clear_proxy_events(&actual_home).map_err(|e| e.to_string())
    })
    .await
}
...
#[tauri::command]
pub async fn check_all_provider_health() -> Result<ProxyState, String> {
    let actual_home = blocking(actual_home).await?;
    crate::proxy::check_all_provider_health(actual_home)
        .await
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn send_proxy_test_request() -> Result<ProxyTestResult, String> {
    let actual_home = blocking(actual_home).await?;
    crate::proxy::send_proxy_test_request(actual_home)
        .await
        .map_err(|e| e.to_string())
}
