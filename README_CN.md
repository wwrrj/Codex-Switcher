...
| 功能 | 说明 |
| --- | --- |
| 多账号管理 | 使用邮箱作为账号名，支持重命名、删除和优先级标记 |
| 安全切换流程 | 静默关闭 Codex、备份 `auth.json`、替换凭据并重新打开 Codex |
| 新账号登录引导 | 当前账号已保存时，先退出登录并等待新认证文件生成 |
| 邮箱与订阅识别 | 从本地认证数据识别邮箱以及 Free / Plus / Pro / Team 订阅 |
| 官方用量查询 | 在启动、切换后和定时任务中查询 Codex 用量窗口 |
| Token 本地统计 | 统计本地会话记录中的输入、缓存输入、输出和推理 Token |
| 用量分析 | 每日热力图、今日 Token、总 Token 和动态 14 天柱状图 |
| 自动备份与设置 | 支持自定义 Codex Home、备份保留数量、刷新间隔和主题 |
| 智能切换 | 基于健康度、优先级和剩余额度推荐切换目标 |
| 托盘与通知 | 后台驻留、托盘快捷切换，并在达到阈值时通知 |
| 切换历史 | 记录每次切换的来源、目标、时间和结果 |
